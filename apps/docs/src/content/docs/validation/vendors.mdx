...
```ts {1-6}
const products = defineQueryModel({
  search: param.text().refine(fromStandardSchema(type("string").pipe((value) => value.length))),
});
...
const priceFilters = defineQueryModel(
  { minPrice: param.number({ min: 0 }), maxPrice: param.number({ min: 0 }) },
  {
    refine: [
      fromStandardSchema(
        v.pipe(
          v.object({ minPrice: v.number(), maxPrice: v.number() }),
          v.check((filters) => filters.minPrice <= filters.maxPrice),
        ),
      ),
    ],
  },
...
const priceFilters = defineQueryModel(
  { minPrice: param.number({ min: 0 }), maxPrice: param.number({ min: 0 }) },
  {
    refine: [
      fromStandardSchema(
        type({ minPrice: "number", maxPrice: "number" }).narrow(
