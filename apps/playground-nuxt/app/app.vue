...
const catalogueFilters = defineQueryModel({
...
const filters = useQueryModel(catalogueFilters);
...
const canonicalQuery = computed(() => {
  const query = formatQueryString(catalogueFilters.encode(filters.values));
...
<template>
  <main class="catalogue-shell">
    <header class="app-header">
      <div>
        <p class="product-mark"><span aria-hidden="true">QW</span> Nuxt module</p>
        <h1>QueryWeave Nuxt playground</h1>
        <p>One request-scoped ecommerce model across SSR, hydration, and navigation.</p>
      </div>
      <div class="connection">{{ filters.status }} · {{ filters.issues.length }} issue(s)</div>
    </header>
    ...
    <div class="catalogue-layout">
      <aside class="filter-panel" aria-labelledby="filters-heading">
        <div class="panel-heading">
          <div>
            ...
            <p>Server-decoded values become writable bindings.</p>
          </div>
          <button type="button" class="text-button" @click="filters.reset()">Clear all</button>
        </div>
        ...
        <fieldset>
          ...
          <div class="toggle-list">
            <label class="switch-row"
              ...
              ><input v-model="inStock" type="checkbox" role="switch" /></label
            ><label class="switch-row"
              ><span><strong>Free shipping</strong><small>No delivery fee</small></span
              ><input v-model="freeShipping" type="checkbox" role="switch" /></label
            ><label class="switch-row"
      ...
      <section class="results-panel" aria-labelledby="state-heading">
        <div class="results-toolbar">
          <div>
            <h2 id="state-heading">Request-aware catalogue state</h2>
            <p>The same values render on the server and update in the browser.</p>
          </div>
        ...
        <div class="inspector">
          <div class="inspector-heading">
            <span>Canonical query</span><span>SSR-safe output</span>
          </div>
          <output class="query-output">{{ canonicalQuery }}</output>
          <div class="inspector-heading">
            <span>Decoded values</span><span>Hydrated model</span>
          </div>
          <pre>{{ values }}</pre>
        ...
        <div class="transition-lab">
          <div>
            <h3>Navigation lab</h3>
            <p>Every change is reflected in the route query.</p>
          </div>
          <div class="action-row">
            ...
            >
              Next page <small>push</small></button
            ><button
              type="button"
              class="action"
              @click="
              ...
              "
            >
              Search “vue” <small>replace</small></button
            ><button
              type="button"
              class="action quiet"
              @click="filters.remove(['search', 'brands'])"
            >
              Remove search + brands
            </button>
          </div>
        </div>
        <span class="sr-only" data-testid="page">{{ filters.values.page }}</span
        ><span class="sr-only" data-testid="search-value">{{
